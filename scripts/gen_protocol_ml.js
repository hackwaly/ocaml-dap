...
for (let key of Object.keys(schema.definitions)) {
  ...
  if (key.endsWith("Event") && key !== "Event") {
    ...
    const cmd = key
      .slice(0, -"Request".length)
      .replace(/^[A-Z]/, (c) => c.toLowerCase());
    requests[cmd] = requests[cmd] || {};
    requests[cmd].doc = def.allOf[1].description;
    requests[cmd].arguments = resolveDef(
      (def.allOf[1].properties || {}).arguments
    );
  } else if (
    ...
    const cmd = key
      ...
      .replace(/^[A-Z]/, (c) => c.toLowerCase());
    requests[cmd] = requests[cmd] || {};
    requests[cmd].responseBody = resolveDef(
      (def.allOf[1].properties || {}).body
    );
...
function genType(def, prop, parentDef) {
  ...
  if (def.oneOf != null) {
    return `[\n${def.oneOf
      .map((it) => {
        const modId0 = toOcamlName(/[^/]+$/.exec(it.$ref)[0]);
        const modId1 = modId0.replace(
          /_request_arguments$/,
          "_command_arguments"
        );
        const modId2 = modId0.replace(
          /_request_arguments$/,
          "_command.Arguments"
        );
        return `  | \`${modId1} of ${modId2}.t`;
      })
      .join("\n")}\n  ]`;
  }
  ...
  if (
    ...
    if (
      parentDef.allOf != null &&
      resolveDef(parentDef.allOf[0]) === types.ProtocolMessage
    ) {
...
function emitTypeDecl(emit, def, { generic, isEmitTypeModule } = {}) {
  ...
  if (!isEmitTypeModule && Object.entries(types).some((it) => it[1] === def)) {
    ...
    if (
      _.isEqual(_.sortBy(def.additionalProperties.type), ["null", "string"])
    ) {
      ...
      emit(` Any_dict.t\n`);
    } else if (def.additionalProperties === false) {
      emit(` Empty_dict.t\n`);
    } else {
      throw new Error("Assertion failed");
    }
    ...
    def == null ||
    (def.type === "object" && _.isEmpty(def.properties))
  ) {
    emit(` Empty_dict.t\n`);
    emit(`[@@deriving yojson]`);
    ...
    withIndent(emit, (emit) => {
      for (const [prop, propDef0] of Object.entries(objDef.properties || {})) {
        ...
        if (
          isOptional &&
          !typExp.endsWith(" option") &&
          !(generic && ["arguments", "body"].includes(prop))
        ) {
          emit(` option`);
        }
        if (mlprop !== prop) {
...
function emitTypeModule(typeName, def) {
  ...
  withBuffer(emit, (emit) => {
    ...
    emitModule(emit, modName, (emit) => {
      emitTypeDecl(emit, def, {
        generic: ["Event", "Request", "Response"].includes(modName),
        isEmitTypeModule: true,
      });
    });
    emit("\n");
  });
