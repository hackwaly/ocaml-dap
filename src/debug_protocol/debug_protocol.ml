...
module Breakpoint_mode_applicability = struct
...
module Breakpoint_mode = struct
  ...
  type t = {
    ...
    description : string option [@default None]; (** A help text providing additional information about the breakpoint mode. This string is typically shown as a hover and can be translated. *)
    applies_to : Breakpoint_mode_applicability.t list [@key "appliesTo"]; (** Describes one or more type of breakpoint this mode applies to. *)
  }
  [@@deriving make, yojson {strict = false}]
end
...
module Capabilities = struct
  ...
  type t = {
    ...
    supports_data_breakpoint_bytes : bool option [@key "supportsDataBreakpointBytes"] [@default None]; (** The debug adapter supports the `asAddress` and `bytes` fields in the `dataBreakpointInfo` request. *)
    breakpoint_modes : Breakpoint_mode.t list option [@key "breakpointModes"] [@default None]; (** Modes of breakpoints supported by the debug adapter, such as 'hardware' or 'software'. If present, the client may allow the user to select a mode and include it in its `setBreakpoints` request.

    Clients may present the first applicable mode in this array as the 'default' mode in gestures that set breakpoints. *)
    supports_ansistyling : bool option [@key "supportsANSIStyling"] [@default None]; (** The debug adapter supports ANSI escape sequences in styling of `OutputEvent.output` and `Variable.value` fields. *)
  }
  [@@deriving make, yojson {strict = false}]
end
...
module Stack_frame = struct
  ...
  type t = {
    ...
    can_restart : bool option [@key "canRestart"] [@default None]; (** Indicates whether this frame can be restarted with the `restartFrame` request. Clients should only use this if the debug adapter supports the `restart` request and the corresponding capability `supportsRestartFrame` is true. If a debug adapter has this capability, then `canRestart` defaults to `true` if the property is absent. *)
...
module Scope = struct
  module Presentation_hint = struct
    ...
    let of_yojson = function
      ...
      | `String "returnValue" -> Ok Return_value
      | `String str -> Ok (Custom str)  | _ -> Error (print_exn_at_loc [%here])
    ...
    let to_yojson = function
      ...
      | Return_value -> `String "returnValue"
      | Custom str -> `String str
  end
...
module Variable = struct
  ...
  type t = {
    ...
    The client can use this information to present the children in a paged UI and fetch them in chunks. *)
    memory_reference : string option [@key "memoryReference"] [@default None]; (** A memory reference associated with this variable.
    ...
    declaration_location_reference : int option [@key "declarationLocationReference"] [@default None]; (** A reference that allows the client to request the location where the variable is declared. This should be present only if the adapter is likely to be able to resolve the location.
    ...
    value_location_reference : int option [@key "valueLocationReference"] [@default None]; (** A reference that allows the client to request the location where the variable's value is declared. For example, if the variable contains a function pointer, the adapter may be able to look up the function's location. This should be present only if the adapter is likely to be able to resolve the location.
...
module Source_breakpoint = struct
  ...
  type t = {
    ...
    mode : string option [@default None]; (** The mode of this breakpoint. If defined, this must be one of the `breakpointModes` the debug adapter advertised in its `Capabilities`. *)
  }
  [@@deriving make, yojson {strict = false}]
end
...
module Instruction_breakpoint = struct
  ...
  type t = {
    ...
    offset : int option [@default None]; (** The offset from the instruction reference in bytes.
...
module Breakpoint = struct
  ...
  type t = {
    ...
    This can be negative. *)
    reason : Reason.t option [@default None]; (** A machine-readable explanation of why a breakpoint may not be verified. If a breakpoint is verified or a specific reason is not known, the adapter should omit this property. Possible values include:
...
module Exception_filter_options = struct
  ...
  type t = {
    ...
    mode : string option [@default None]; (** The mode of this exception breakpoint. If defined, this must be one of the `breakpointModes` the debug adapter advertised in its `Capabilities`. *)
  }
  [@@deriving make, yojson {strict = false}]
end
...
module Disassembled_instruction = struct
  ...
  type t = {
    ...
    presentation_hint : Presentation_hint.t option [@key "presentationHint"] [@default None]; (** A hint for how to present the instruction in the UI.
...
module Output_event = struct
  ...
  module Payload = struct
    ...
    type t = {
      ...
      location_reference : int option [@key "locationReference"] [@default None]; (** A reference that allows the client to request the location where the new value is declared. For example, if the logged value is function pointer, the adapter may be able to look up the function's location. This should be present only if the adapter is likely to be able to resolve the location.
...
module Process_event = struct
  ...
  module Payload = struct
    ...
    type t = {
      ...
      system_process_id : int option [@key "systemProcessId"] [@default None]; (** The process ID of the debugged process, as assigned by the operating system. This property should be omitted for logical processes that do not map to operating system processes on the machine. *)
...
module Start_debugging_command = struct
  ...
  module Arguments = struct
    ...
    module Output_presentation = struct
      ...
      let of_yojson = function
        | `String "separate" -> Ok Separate
        | `String "mergeWithParent" -> Ok Merge_with_parent
        | _ -> Error (print_exn_at_loc [%here])

      let to_yojson = function
        | Separate -> `String "separate"
        | Merge_with_parent -> `String "mergeWithParent"
...
module Initialize_command = struct
  ...
  module Arguments = struct
    ...
    type t = {
      ...
      supports_ansistyling : bool option [@key "supportsANSIStyling"] [@default None]; (** The client will interpret ANSI escape sequences in the display of `OutputEvent.output` and `Variable.value` fields when `Capabilities.supportsANSIStyling` is also enabled. *)
    }
    [@@deriving make, yojson {strict = false}]
  end
...
module Data_breakpoint_info_command = struct
  ...
  module Arguments = struct
    ...
    type t = {
      ...
      bytes : int option [@default None]; (** If specified, a debug adapter should return information for the range of memory extending `bytes` number of bytes from the address or variable specified by `name`. Breakpoints set using the resulting data ID should pause on data access anywhere within that range.

      Clients may set this property only if the `supportsDataBreakpointBytes` capability is true. *)
      as_address : bool option [@key "asAddress"] [@default None]; (** If `true`, the `name` is a memory address and the debugger should interpret it as a decimal value, or hex value if it is prefixed with `0x`.

      Clients may set this property only if the `supportsDataBreakpointBytes`
      capability is true. *)
      mode : string option [@default None]; (** The mode of the desired breakpoint. If defined, this must be one of the `breakpointModes` the debug adapter advertised in its `Capabilities`. *)
    }
    [@@deriving make, yojson {strict = false}]
  end
...
module Set_variable_command = struct
  ...
  module Result = struct
    type t = {
      ...
      This attribute may be returned by a debug adapter if corresponding capability `supportsMemoryReferences` is true. *)
      value_location_reference : int option [@key "valueLocationReference"] [@default None]; (** A reference that allows the client to request the location where the new value is declared. For example, if the new value is function pointer, the adapter may be able to look up the function's location. This should be present only if the adapter is likely to be able to resolve the location.
...
module Evaluate_command = struct
  ...
  module Arguments = struct
    ...
    type t = {
      ...
      line : int option [@default None]; (** The contextual line where the expression should be evaluated. In the 'hover' context, this should be set to the start of the expression being hovered. *)
      column : int option [@default None]; (** The contextual column where the expression should be evaluated. This may be provided if `line` is also provided.

      It is measured in UTF-16 code units and the client capability `columnsStartAt1` determines whether it is 0- or 1-based. *)
      source : Source.t option [@default None]; (** The contextual source in which the `line` is found. This must be provided if `line` is provided. *)
  ...
  module Result = struct
    type t = {
      ...
      For pointer type eval results, this is generally a reference to the memory address contained in the pointer.
      This attribute may be returned by a debug adapter if corresponding capability `supportsMemoryReferences` is true. *)
      value_location_reference : int option [@key "valueLocationReference"] [@default None]; (** A reference that allows the client to request the location where the returned value is declared. For example, if a function pointer is returned, the adapter may be able to look up the function's location. This should be present only if the adapter is likely to be able to resolve the location.
...
module Locations_command = struct
  let type_ = "locations"

  module Arguments = struct
    (** Arguments for `locations` request. *)
    type t = {
      location_reference : int [@key "locationReference"]; (** Location reference to resolve. *)
    }
    [@@deriving make, yojson {strict = false}]
  end
  ...
  module Result = struct
    type t = {
      source : Source.t; (** The source containing the location; either `source.path` or `source.sourceReference` must be specified. *)
      line : int; (** The line number of the location. The client capability `linesStartAt1` determines whether it is 0- or 1-based. *)
      column : int option [@default None]; (** Position of the location within the `line`. It is measured in UTF-16 code units and the client capability `columnsStartAt1` determines whether it is 0- or 1-based. If no column is given, the first position in the start line is assumed. *)
      end_line : int option [@key "endLine"] [@default None]; (** End line of the location, present if the location refers to a range.  The client capability `linesStartAt1` determines whether it is 0- or 1-based. *)
      end_column : int option [@key "endColumn"] [@default None]; (** End position of the location within `endLine`, present if the location refers to a range. It is measured in UTF-16 code units and the client capability `columnsStartAt1` determines whether it is 0- or 1-based. *)
    }
    [@@deriving make, yojson {strict = false}]
  end
end
